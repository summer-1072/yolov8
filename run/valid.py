...
if __name__ == "__main__":
    ...
    grid_stride = torch.tensor([[4], [2], [2], [2], [2]])

    img_infos = [{'shape': (4, 4), 'ratio': 1, 'offset': (0, 0)},
                 {'shape': (4, 4), 'ratio': 1, 'offset': (0, 0)}]

    hyp = {'alpha': 1, 'beta': 1, 'topk': 1, 'box_w': 1, 'cls_w': 1, 'dfl_w': 1, 'conf_t': 0.25,
           'multi_label': False, 'max_box': 30000, 'max_wh': 7680, 'iou_t': 0.7, 'max_det': 300, 'merge': False}

    device = 'cpu'

    loss_fun = LossFun(hyp['alpha'], hyp['beta'], hyp['topk'], hyp['box_w'], hyp['cls_w'], hyp['dfl_w'], 4, device)
